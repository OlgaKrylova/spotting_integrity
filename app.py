...
if ss.page == "upload":
    ...
    if uploads:
        ...
        if "equipment_status_trans" in dfs:
            ...
            ts_min = pd.to_datetime(eq["START_TIMESTAMP"], errors="coerce").min() if "START_TIMESTAMP" in eq.columns else None
            ts_max = pd.to_datetime(eq["END_TIMESTAMP"], errors="coerce").max() if "END_TIMESTAMP" in eq.columns else None
